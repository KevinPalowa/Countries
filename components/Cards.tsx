...
const Cards = ({items,query}:Props) => {
    const filtered = items.filter((country)=>{
      if(query.search.length > 0 && query.region.length > 0){
        return  country.name.common.toLowerCase().includes(query.search.toLowerCase()) && country.region.toLowerCase() === query.region.toLowerCase()
      }else if(query.search.length > 0 && query.region.length === 0){
        return  country.name.common.toLowerCase().includes(query.search.toLowerCase())      
      }else if(query.search.length === 0 && query.region.length > 0){
